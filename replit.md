...
- **Marketing & Onboarding**: Conversion-optimized landing page and a 3-step onboarding flow. Schema: `onboardingStep` (0-3), `onboardingDismissed` (boolean), `activatedAt` (timestamp) on users table. Steps: (1) Complete Setup — fill restaurant details in HandbookBuilder, min fields: restaurantName, ownerNames, one manager; (2) Generate Manual — export any training manual from templates page; (3) Ask Consultant — send first message. Components: `WelcomeOverlay.tsx` (full-page dark overlay on dashboard for new users), `OnboardingProgressBar.tsx` (persistent 3-step bar at top of all pages, auto-dismisses on completion after 6s). Page enhancements: Setup banner on HandbookBuilder, spotlight card + post-gen modal variant on templates, onboarding empty state + suggestion chips on consultant. Returning user nudge cards on dashboard for incomplete steps. API: `GET /api/onboarding/status`, `POST /api/onboarding/step` (sequential validation), `POST /api/onboarding/dismiss`. Hook: `use-onboarding.ts`. Analytics: `onboardingAnalytics.ts` (console.log events). Existing users backfilled with `onboarding_dismissed=true`.

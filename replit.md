...
- **Freemium Subscription Tiers**: Three-tier pricing model (Free/Basic/Pro) with domain-based feature gating. Free tier: 3 domains (leadership, training, crisis). Basic ($10/mo or $99/yr): all 12 domains. Pro ($25/mo or $249/yr): data export, priority support, advanced analytics. Managed via `subscriptionTier` column on users table, `useTierAccess` hook, and `SubscriptionGate` component with `requirePaid` prop. Pricing page at `/pricing` with monthly/annual toggle. Stripe checkout creates tier-specific products/prices dynamically using lookup keys. Webhook handler auto-updates tier on subscription changes and downgrades to "free" on cancellation.

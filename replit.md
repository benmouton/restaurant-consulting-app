...
**Important:** The word "AI" must never appear anywhere a user can see it. The technology is invisible. The expertise is the product. System prompts in backend code are the only exception.
...
- **Freemium Subscription Tiers**: Three-tier pricing model (Free/Basic/Pro) with domain-based feature gating. Free tier: 3 domains (leadership, kitchen, crisis). Basic ($10/mo or $99/yr): all 12 domains + consultant + tools. Pro ($25/mo or $249/yr): data export, priority support, advanced analytics. Managed via `subscriptionTier` column on users table, `useTierAccess` hook, `UpgradeGate` component (blur overlay for gated domain pages), and `SubscriptionGate` component with `requirePaid` prop. Tier config defined in `client/src/config/tierConfig.ts`. Pricing page at `/pricing` with monthly/annual toggle and domain comparison grid. Stripe checkout creates tier-specific products/prices dynamically using lookup keys. Webhook handler auto-updates tier on subscription changes and downgrades to "free" on cancellation.
...
- **Enhanced Content Accordion**: Domain content grouped by type (Principle, Framework, Checklist, Script) with enhanced rendering: interactive checkboxes for checklists, numbered step flows for scripts, role-badge sub-cards (SERVER/BARTENDER/HOST etc.) for frameworks, animated expand/collapse. Service domain includes a `ServiceMetricStrip` (comp budget, active standards, recovery protocols), `GuestRecoveryAdvisor` with gold-accented UI, and a Quick Reference tab showing top recovery protocols, timing standards, and comp authority limits. Components: `ServiceContentAccordion`, `ServiceQuickReference`, `ServiceMetricStrip`, `GuestRecoveryAdvisor`, `renderChecklistContent`, `renderScriptContent`, `renderFrameworkContent` — all in `domain.tsx`.

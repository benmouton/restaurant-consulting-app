# Replit Prompt — SOP Generator

Paste this directly into Replit chat:

---

Build an **SOP Generator** as a new module within the Service Standards & SOPs domain. This is a new feature — not a redesign of anything existing. It generates short, formatted, printable Standard Operating Procedure documents personalized to the operator's restaurant using their Setup data — the same variable injection architecture already used by the training manuals.
...
**DO NOT CHANGE:**
- Any existing Service Standards content
- Training manual templates or generation logic
...
An operator selects an SOP from a library of 16 pre-built templates, clicks Generate, and receives a clean 1–2 page document personalized to their restaurant with their name, management team, POS system, and operational details already filled in. The document is immediately printable, exportable as PDF, and shareable as a .docx file.

SOPs can be regenerated at any time if procedures change. Every generated SOP is saved to the operator's SOP Library so they build a complete, living operations manual over time.
...
CREATE TABLE generated_sops (
  ...
  sop_key VARCHAR(100) NOT NULL,        -- e.g. 'tabc-compliance', 'bar-opening'
  sop_title VARCHAR(200) NOT NULL,
  sop_category VARCHAR(50) NOT NULL,    -- bar, kitchen, foh, management
  content TEXT NOT NULL,                -- rendered HTML content of the SOP
  version INTEGER DEFAULT 1,           -- increments on each regeneration
  last_generated_at TIMESTAMP DEFAULT NOW(),
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, sop_key)             -- one active SOP per type per operator
);
```
...
```
GET    /api/sops                     — all generated SOPs for current user
POST   /api/sops/generate            — generate or regenerate an SOP by key
DELETE /api/sops/:id                 — delete a generated SOP
GET    /api/sops/export/:id          — export individual SOP as PDF or docx
GET    /api/sops/export/all          — export all SOPs as ZIP
```

On POST `/api/sops/generate`:
- Pull operator's full Setup record
- Inject all variables into the requested SOP template
- Render the content server-side
- Save to `generated_sops` (upsert on sop_key — increment version if already exists)
...
**Left panel (30%):** SOP Library — categorized list of all 16 SOP templates
**Right panel (70%):** Selected SOP — template preview before generation, rendered document after generation

On mobile: stacked, library on top, document below.
...
Title: **"SOP Library"**
Subtitle (muted gray): *"Select an SOP to generate. Each one is personalized to `{{restaurantName}}`."*

Four category sections, each collapsible with a gold chevron:

**🍸 Bar (4 SOPs)**
**🍳 Kitchen (4 SOPs)**
**🍽️ Front of House (4 SOPs)**
**📋 Management (4 SOPs)**

Each SOP in the list renders as a row:

```
[ icon ]  TABC Compliance Protocol
          Alcohol service standards and cut-off procedure
          ✓ Generated Mar 6  [ Regenerate ]
          — Not yet generated  [ Generate ]
```

- Generated SOPs: gold checkmark, generation date, "Regenerate" ghost button
- Not yet generated: dash, muted text, gold "Generate" button
- Clicking a row opens that SOP's preview or rendered document in the right panel
- A progress indicator at the top of the left panel: "4 of 16 SOPs generated" with a thin gold progress bar

---

### RIGHT PANEL: SOP PREVIEW / DOCUMENT VIEW

**Before generation — Template Preview:**

Show a preview card describing what the SOP covers and what Setup variables it will use:

```
TABC Compliance Protocol
Bar · 1–2 pages · ~3 min read

What this SOP covers:
- Legal service standards under {{alcoholPermit}}
- ID verification requirements
- Intoxication recognition and cut-off protocol
- Refused service documentation
- Incident log procedure

Variables this SOP uses from your Setup:
✓ Restaurant name          ✓ Alcohol permit type
✓ Bar manager name         ✓ Closing time
⚠ Emergency contacts       ← not yet set in Setup

[ Generate SOP ]
```

If any required variables are missing from Setup: show them as amber ⚠ with a link to complete Setup. The Generate button is still active — missing variables render as "[NOT SET — complete in Setup]" placeholders in the document.

**After generation — Rendered Document:**

Show the full SOP content in a clean document view inside the right panel. This is the same document that will print/export.

Document action bar pinned to top of right panel:
```
[ 🖨️ Print ]   [ ⬇️ Download PDF ]   [ 📤 Share / Export .docx ]   [ 🔄 Regenerate ]
```

Same print/export/share architecture already built for the training manuals. Reuse the existing `exportUtils.js` functions.

---

## THE 16 SOP TEMPLATES

Each SOP template is a structured document that injects Setup variables and follows a consistent format:
...
Effective Date: [generation date]
...
{{restaurantName}} · Last updated: [generation date]
...
**SOP 1: TABC Compliance Protocol**
Key: `tabc-compliance`

```
PURPOSE
To ensure all bar staff at {{restaurantName}} serve alcohol in full compliance
with Texas Alcoholic Beverage Commission regulations under permit {{alcoholPermit}}.
...
Log location: [operator to specify — physical logbook or digital record]
Log must include: date, time, guest description, reason for refusal, staff name, manager sign-off

VIOLATIONS
Any violation of this SOP — including serving a minor or visibly intoxicated guest —
is grounds for immediate termination and may result in personal TABC license suspension.
...
**SOP 2: Bar Opening Procedure**
Key: `bar-opening`

```
PURPOSE
To ensure the bar at {{restaurantName}} opens in a consistent, fully prepared
state for every service period.
...
☐ Restock garnish station: citrus cut, cherries, olives, other [operator to define]
...
**SOP 3: Bar Closing Procedure**
Key: `bar-closing`

```
PURPOSE
To ensure the bar at {{restaurantName}} closes consistently, safely, and
fully prepared for the next service.
...
**SOP 4: Intoxicated Guest Protocol**
Key: `intoxicated-guest`

```
PURPOSE
To protect {{restaurantName}}, its staff, and its guests by providing a
clear, consistent response to intoxicated guests that minimizes liability
and ensures safety.

IDENTIFICATION
Any staff member — not just bar staff — may identify and report a guest
who appears intoxicated. Report immediately to {{barManager}} or manager on duty.

RESPONSE PROTOCOL

STEP 1: STOP SERVICE
Cease alcohol service immediately. Do not wait for manager confirmation
before stopping — you can always resume service if the assessment is wrong.
You cannot un-serve a drink.

STEP 2: NOTIFY MANAGER
Alert {{gm}} or manager on duty before any conversation with the guest.
...
Immediately offer: water, food, non-alcoholic beverage.
Do not wait to be asked. Proactive offers de-escalate.

STEP 5: TRANSPORTATION
Do not allow an intoxicated guest to drive.
Offer to call a rideshare. If guest refuses and attempts to drive:
call 911. Document that the offer was made and refused.
...
ESCALATION
If the guest becomes physically aggressive: call 911 immediately.
Do not physically restrain. Clear other guests from the area.
Notify {{ownerName}} same night.
...
**SOP 5: Health Inspection Preparation**
Key: `health-inspection-prep`

```
PURPOSE
To ensure {{restaurantName}} is prepared to pass a health inspection
at any time — not just when one is scheduled.

PHILOSOPHY
A health inspection should find nothing. The standard in this kitchen
is not "clean enough to pass." It is "clean enough that an inspector
walking in unannounced finds nothing to write."
...
WEEKLY SELF-AUDIT
Every [operator to define: Sunday / Monday] before service:
Manager conducts a self-inspection using the health department's
published checklist for {{restaurantName}}'s jurisdiction.
Findings documented and corrected before service begins.
```

---

**SOP 6: Vendor Receiving Procedure**
Key: `vendor-receiving`

```
PURPOSE
To ensure all deliveries received at {{restaurantName}} meet quality,
quantity, and food safety standards before acceptance.
...
STEP 1: COUNT
Count every item against the invoice before signing.
Shortages must be noted on the delivery receipt before the driver leaves.
Never sign for product you haven't counted.
...
STEP 4: PRICE CHECK
Verify invoice prices against the most recent agreed pricing.
Any price increase greater than [operator to define]% requires
manager approval before acceptance — do not silently absorb increases.
Flag all discrepancies to {{ownerName}} within 24 hours.

STEP 5: SIGN AND STORE
Sign the delivery receipt only after steps 1–4 are complete.
Note any rejections or discrepancies on the receipt.
Store product immediately using FIFO: new product behind existing stock.
All items labeled with received date before going into storage.
...
**SOP 7: Temperature Log Procedure**
Key: `temperature-log`

```
PURPOSE
To maintain a consistent, accurate temperature log at {{restaurantName}}
that satisfies health code requirements and protects against food safety incidents.
...
☐ Any other refrigerated or heated storage [operator to add]

LOG PROCEDURE
1. Use a calibrated probe thermometer — verify calibration weekly
2. Record the actual temperature reading — never estimate
3. If a unit is out of range: do not log and walk away. Follow the
   out-of-range protocol below immediately.
...
LOG STORAGE
Temperature logs are retained for a minimum of 90 days.
Location: [operator to define — physical binder or digital record]
Available for health inspector review at all times.
```

---

**SOP 8: Station Close-Down Procedure**
Key: `station-closedown`

```
PURPOSE
To ensure every kitchen station at {{restaurantName}} closes consistently,
safely, and fully prepared for the next service.

RESPONSIBLE PARTY
Each cook owns their station close. Manager on duty signs off before any
BOH staff clocks out.
...
**SOP 9: New Hire Onboarding Procedure**
Key: `new-hire-onboarding`

```
PURPOSE
To ensure every new employee at {{restaurantName}} has a consistent,
professional first day that sets clear expectations from the start.
...
ORIENTATION PERIOD
{{orientationPeriod}} — performance is evaluated continuously.
Any conduct, attendance, or performance issue during orientation
may result in immediate termination without progressive discipline.
...
**SOP 10: Reservation Handling Procedure**
Key: `reservation-handling`

```
PURPOSE
To ensure all reservations at {{restaurantName}} are taken, confirmed,
and managed consistently using {{reservationSystem}}.
...
Reservations of 6+ guests: confirm by phone or text 24 hours in advance.
If confirmation is not reached: hold the reservation, attempt contact once more
2 hours before the reservation time.

SAME-DAY RESERVATIONS
Accept same-day reservations only if availability exists in {{reservationSystem}}.
Do not verbally promise a table without confirming in the system first.

NO-SHOW PROTOCOL
Hold the table for 15 minutes past reservation time.
At 15 minutes: one phone contact attempt.
At 20 minutes with no contact: release the table, mark as no-show in {{reservationSystem}}.
Late arrivals after release: seat as walk-in if available; waitlist with priority if not.
...
CANCELLATIONS
Mark as canceled — do not delete. Manager reviews no-show and cancellation
patterns weekly for operational planning.
```

---

**SOP 11: Large Party Service Procedure**
Key: `large-party`

```
PURPOSE
To ensure large parties at {{restaurantName}} receive consistent,
coordinated service that doesn't compromise the experience of
surrounding guests.

DEFINITION
A large party is any group of 8 or more guests seated at the same time
in the same area.
...
**SOP 12: Guest Complaint Escalation Procedure**
Key: `guest-complaint-escalation`

```
PURPOSE
To ensure every guest complaint at {{restaurantName}} is handled
consistently, documented, and resolved in a way that retains the guest.

PHILOSOPHY
A guest who complains is giving the restaurant a chance to fix something.
A guest who says nothing and leaves unhappy is not. Chase the complaint.
...
Manager: comp up to $[operator to define] without owner approval
...
Response tone: factual, non-defensive, solution-oriented.
Never argue publicly. Acknowledge, apologize for the experience, invite back.
Flag patterns to {{ownerName}}: same complaint 3+ times = operational signal.
```
...
**SOP 13: Cash Handling Procedure**
Key: `cash-handling`

```
PURPOSE
To ensure all cash transactions at {{restaurantName}} are handled
consistently and securely, and that all variances are documented.

OPENING BANK
- Opening bank counted by opening manager and one witness before service
- Amount confirmed: $[operator to define]
...
- Mid-service drawer pull if drawer exceeds $[operator to define] — manager only
...
VARIANCE POLICY
Cash variances are documented, not ignored. Consistent variance on one
employee's shifts is an investigation trigger — not a training issue.
...
**SOP 14: Safe Drop Procedure**
Key: `safe-drop`

```
PURPOSE
To ensure all cash drops at {{restaurantName}} follow a consistent,
documented procedure that protects staff and the business.
...
7. Never drop alone — a witness must be present for all drops over $[operator to define]

SAFE OPENING
Safe may only be opened by {{ownerName}} or {{gm}}.
Safe is opened for: bank replenishment, deposit preparation, emergency only.
Two-person rule for all safe openings above $[operator to define].
...
Deposit schedule: [operator to define — daily / every 2 days / weekly]
```

---

**SOP 15: End-of-Night Manager Checklist**
Key: `end-of-night-manager`

```
PURPOSE
To ensure the closing manager at {{restaurantName}} completes all
required steps before leaving the building.
...
**SOP 16: Employee Discipline Procedure**
Key: `employee-discipline`

```
PURPOSE
To ensure all disciplinary actions at {{restaurantName}} follow a
consistent, documented process that complies with Texas Workforce
Commission standards and protects the business.

CORE PRINCIPLE
If it wasn't written down, it didn't happen.
Every disciplinary action — including verbal warnings — must be documented.
...
- Document immediately after the conversation:
  date, time, violation, what was said, employee's response
...
TWC DOCUMENTATION
All disciplinary records retained for minimum 3 years.
In any TWC unemployment or wrongful termination dispute, these
records are your primary defense. Incomplete documentation = presumption against the employer.
...
## SOP LIBRARY CARD GRID — ALTERNATE ENTRY POINT

In addition to the two-panel layout, add a card grid view option (toggle between list and grid):

Each SOP renders as a card matching the platform's module card style:
- Category icon
- SOP title in white
- One-line description in muted gray
- Status: gold "Generated [date]" or muted "Not generated"
- Gold "Generate" or "Regenerate" button

---

## PRINT / EXPORT BEHAVIOR

Reuse the existing `exportUtils.js` print/PDF/docx functions already built for the training manuals. Each SOP exports as:

- **Print:** clean black on white, 1–2 pages, letter size, 1-inch margins
- **PDF download:** `{{restaurantName}}-[SOP-Title]-[YYYY-MM-DD].pdf`
- **Docx share:** `{{restaurantName}}-[SOP-Title]-[YYYY-MM-DD].docx` — editable, shareable via iOS share sheet

**Bulk export:** "Export All SOPs" button generates a single PDF with all generated SOPs compiled into one document — the operator's complete operations manual. This is the highest-value export in the entire platform. Label it: **"Export Complete Operations Manual (PDF)"**

---

## UPGRADE GATE

Wrap the entire SOP Generator in the existing `<UpgradeGate>` component.

Add to the upgrade gate copy map:
```
'service-standards': {
  headline: "Your Operations Manual — Generated, Not Written",
  sub: "16 SOPs personalized to your restaurant. Health inspection prep, vendor receiving, cash handling, discipline procedure — every document your operation needs, ready in minutes."
}
```
...
1. Database migration — create `generated_sops` table
2. API endpoints — generate, fetch, delete, export
3. SOP template engine — variable injection logic (reuse existing training manual pattern)
4. All 16 SOP templates as server-side template strings with variable injection
5. Left panel — SOP Library with category sections and generation status
6. Right panel — template preview with variable status indicators
7. Right panel — rendered document view with action bar
8. Bulk "Export Complete Operations Manual" PDF
9. Individual print / PDF / docx export (reuse exportUtils.js)
10. SOP card grid view (alternate layout)
11. Navigation — add SOP Generator as primary feature in Service Standards domain
12. Apply UpgradeGate wrapper with new copy
13. Mobile responsiveness pass
14. Smoke test: generate all 16 SOPs with complete Setup, verify variable injection, verify export quality

---

## SUCCESS CRITERIA

- An operator with complete Setup can generate all 16 SOPs without any "[NOT SET]" placeholders
- Each generated SOP is saved and retrievable from the operator's SOP Library
- Regenerating an SOP increments the version number and updates the content
- The "Export Complete Operations Manual" PDF compiles all generated SOPs into a single clean document
- A free-tier user sees the UpgradeGate overlay with correct copy
- All 16 templates inject Setup variables correctly — restaurant name, owner, manager names, POS system, permit type, closing time, and all other relevant fields
- Individual SOPs export as clean, professional black-on-white documents suitable for posting in a restaurant or filing with HR

Make all changes to the Service Standards domain only. Do not touch training manuals, Setup logic, Stripe integration, tier configuration, or any other modules.

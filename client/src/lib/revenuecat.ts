...
export async function getOfferings() {
  ...
  try {
    ...
    // RC Capacitor wraps response in { offerings: { current, all } }
...
export function getEntitlementTier(customerInfo: any): string {
  ...
  if (entitlements["Pro"]) return "pro"; // capital P to match RC dashboard
  if (entitlements["Basic"]) return "basic"; // capital B to match RC dashboard
  return "free";
}
